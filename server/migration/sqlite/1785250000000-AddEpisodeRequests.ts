...
export class AddEpisodeRequests1785250000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "media_request" ADD "episodeSelectionType" varchar`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `UPDATE "media_request" SET "tvQuotaUnits" = (SELECT COUNT(*) FROM "season_request" WHERE "season_request"."requestId" = "media_request"."id") WHERE "type" = 'tv'`
    );
    await queryRunner.query(
      `CREATE TABLE "episode_request" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "tvdbId" integer NOT NULL, "seasonNumber" integer NOT NULL, "episodeNumber" integer NOT NULL, "title" varchar, "airDate" varchar, "status" integer NOT NULL DEFAULT (1), "searchTriggeredAt" datetime, "requestId" integer, "createdAt" datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP), "updatedAt" datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP), CONSTRAINT "UQ_episode_request_request_tvdb" UNIQUE ("requestId", "tvdbId"), CONSTRAINT "FK_episode_request_request" FOREIGN KEY ("requestId") REFERENCES "media_request" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`
    );
    await queryRunner.query(
