...
export class AddActiveOngoingEpisodeRequestConstraint1786200000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "media_request" ADD "ongoingEpisodeRequestKey" varchar`
    );
    await queryRunner.query(
      `UPDATE "media_request" SET "ongoingEpisodeRequestKey" = (SELECT CAST("tmdbId" AS TEXT) || ':' || CASE WHEN "is4k" THEN '4k' ELSE 'sd' END FROM "media" WHERE "media"."id" = "media_request"."mediaId") WHERE "episodeSelectionType" = 'after'`
    );
