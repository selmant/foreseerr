...
export class AddSimklUserSettings1787300000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "user_settings" ADD "simklAccessToken" varchar`
    );
    await queryRunner.query(
      `ALTER TABLE "user_settings" ADD "simklUsername" varchar`
    );
    await queryRunner.query(
      `ALTER TABLE "user_settings" ADD "simklUserId" varchar`
    );
    await queryRunner.query(
