...
describe('expandTmdbGenreIds', () => {
  it('keeps exact IDs and adds the movie/TV pair', () => {
    assert.deepEqual(expandTmdbGenreIds([28]).sort((a, b) => a - b), [
      28, 10759,
    ]);
    assert.deepEqual(expandTmdbGenreIds([10759]).sort((a, b) => a - b), [
      12, 28, 10759,
    ]);
    assert.deepEqual(expandTmdbGenreIds([878]).sort((a, b) => a - b), [
      878, 10765,
    ]);
  });
