  ...
  public async getSeerrReleases({
    ...
    try {
      const data = await this.get<GitHubRelease[]>(
        '/repos/selmant/foreseerr/releases',
        {
          params: {
            per_page: take,
          },
  ...
  public async getSeerrCommits({
    ...
    try {
      const data = await this.get<GithubCommit[]>(
        '/repos/selmant/foreseerr/commits',
        {
          params: {
            per_page: take,
            branch,
      ...
      logger.warn(
        "Failed to retrieve GitHub commits. This may be an issue on GitHub's end. Foreseerr can't check if it's on the latest version.",
        { label: 'GitHub API', errorMessage: e.message }
      );
      return [];
    }
