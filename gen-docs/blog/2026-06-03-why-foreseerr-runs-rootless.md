---
title: "Why Foreseerr Doesn't Support PUID/PGID"
description: "Foreseerr runs rootless by design. Here's why we won't be adding PUID/PGID support, and why chown is the right approach."
slug: why-foreseerr-doesnt-support-puid-pgid
authors: [fallenbagel]
image: https://raw.githubusercontent.com/selmant/foreseerr/refs/heads/develop/gen-docs/static/img/logo_full.svg
hide_table_of_contents: false
---

A common question we get from users migrating from Overseerr or Jellyseerr is why Foreseerr doesn't support `PUID`/`PGID` environment variables for setting the user the container runs as. This post explains the reasoning behind that decision and why using chown on the host is the correct approach.
...
## Why does Foreseerr default to UID/GID `1000:1000`?
...
However, if a security vulnerability allows an attacker to escape the container, they inherit the permissions of that UID/GID on the host, if it exists. This is exactly the scenario we're avoiding by not running as root. If Foreseerr (or any of its dependencies) ran as root, as Overseerr and Jellyseerr did, a container escape would hand an attacker root on your host. The same applies to the `PUID`/`PGID` pattern: since it requires the container to start as root before dropping privileges, a vulnerability triggered before that drop would have the same effect. To mitigate this, Docker offers [user namespace remapping](https://docs.docker.com/engine/security/userns-remap/), which lets you map container UIDs to different, non-conflicting UIDs on the host for stronger isolation.
...
As for why we don't go into more depth on this in our docs: we focus on the basics and the most common use cases for Foreseerr itself. Docker's permission model, including how the `USER` directive works, is already well documented by Docker. Our goal is to get you up and running smoothly, while encouraging you to explore the tools you're using.

## So what do you actually need to do?

If you're mounting a data directory into the Foreseerr container, that directory needs to be owned by UID `1000` (the `node` user) on the host. You do this once:

```bash
chown -R 1000:1000 /your/foreseerr/data
```
...
**Foreseerr's container setup is more secure than what Overseerr and Jellyseerr shipped, and we intend to keep it that way.**