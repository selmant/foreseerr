---
title: Reverse Proxy
description: Configure a reverse proxy for Foreseerr.
sidebar_position: 1
---
...
:::warning
Base URLs cannot be configured in Foreseerr. With this limitation, only subdomain configurations are supported.
...
:::warning
This Nginx subfolder reverse proxy is an unsupported workaround, and only provided as an example. The filters may stop working when Foreseerr is updated.

If you encounter any issues with Foreseerr while using this workaround, we may ask you to try to reproduce the problem without the Nginx proxy.
:::
...
- **Domain Names:** Your desired external Foreseerr hostname; e.g., `seerr.example.com`
- **Scheme:** `http`
- **Forward Hostname / IP:** Internal Foreseerr hostname or IP
- **Forward Port:** `5055`
- **Cache Assets:** yes
...
Add the following labels to the Foreseerr service in your `compose.yaml` file:
...
:::warning
This Apache2 subfolder reverse proxy is an unsupported workaround, and only provided as an example. The filters may stop working when Foreseerr is updated.

If you encounter any issues with Foreseerr while using this workaround, we may ask you to try to reproduce the problem without the Apache2 proxy.
:::
