...
description: Configure the database for Foreseerr
sidebar_position: 2
---
# Configuring the Database

Foreseerr supports SQLite and PostgreSQL. The database connection can be configured using the following environment variables:
...
DB_CONNECT_TIMEOUT_MS="30000" # (optional) Maximum wait for a free pool connection in milliseconds. The default is "30000"; "0" waits indefinitely.
DB_LOG_QUERIES="false" # (optional) Whether to log the DB queries for debugging. The default is "false".
```
...
1. Set up your PostgreSQL database and configure Foreseerr to use it
2. Run Foreseerr to create the tables in the PostgreSQL database
3. Stop Foreseerr
...
**Recommended method**: Use the pgloader container even for standalone Foreseerr installations. This avoids building from source and ensures compatibility.
