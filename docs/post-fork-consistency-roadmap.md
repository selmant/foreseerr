...
| AniList discovery | App credentials, per-user PIN linking, public catalog (trending/season/popular/top/next-season), linked watching/planning/completed and named lists, TMDB mapping | `server/api/anilist/`, `server/lib/anilist/`, `server/routes/discover.ts`, `src/components/Discover/DiscoverAnilist*`, `src/pages/discover/anilist/` | Catalog rows shipped; unmapped titles are dropped |
...
| MDBList public lists | Pin public lists as custom Discover sliders via search or URL/`user/slug`/id; shared instance API key and quota with badges | `server/api/mdblist/lists.ts`, `server/routes/discover.ts`, `src/components/Discover/MdblistListSlider`, `src/pages/discover/mdblist/` | Public pin only; no My Lists or personalized recs |
