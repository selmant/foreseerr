...
description: How to configure Foreseer to work with services that use self-signed SSL certificates.
---
...
If your media server or services (Radarr, Sonarr, etc.) use self-signed SSL certificates, Foreseer will reject the connection because it does not trust them by default. The fix is to add your CA certificate to Node.js.
