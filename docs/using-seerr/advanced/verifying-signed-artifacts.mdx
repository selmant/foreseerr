...
- GitHub Container Registry (GHCR): `ghcr.io/selmant/foreseerr:<tag>`
- Docker Hub: `selmantr/foreseerr:<tag>`

You can view all available tags on the [Foreseerr Releases page](https://github.com/selmant/foreseerr/releases).

---

### Verifying a Specific Release Tag

Each tagged release (for example `v0.7.0`) is immutable and cryptographically signed.
...
```bash
docker buildx imagetools inspect ghcr.io/selmant/foreseerr:v0.7.0 --format '{{json .Manifest.Digest}}' | tr -d '"'
```
  </TabItem>
...
```bash
skopeo inspect docker://ghcr.io/selmant/foreseerr:v0.7.0 --format '{{.Digest}}'
```
  </TabItem>
</Tabs>
...
cosign verify ghcr.io/selmant/foreseerr@sha256:abcd1234... \
  --certificate-identity "https://github.com/selmant/foreseerr/.github/workflows/release.yml@refs/tags/v0.7.0" \
...
- GitHub Container Registry (GHCR): `ghcr.io/selmant/foreseerr/foreseerr-chart:<tag>`

You can view all available tags on the [Foreseerr package page](https://github.com/selmant/foreseerr/pkgs/container/foreseerr%2Fforeseerr-chart).

---

### Verifying a Specific Release Tag

Each published chart version (for example `0.9.0`) is cryptographically signed.
Verification should always be performed using the chart digest (SHA256).
...
```bash
docker buildx imagetools inspect ghcr.io/selmant/foreseerr/foreseerr-chart:0.9.0 --format '{{json .Manifest.Digest}}' | tr -d '"'
```
  </TabItem>
...
```bash
skopeo inspect docker://ghcr.io/selmant/foreseerr/foreseerr-chart:0.9.0 --format '{{.Digest}}'
```
  </TabItem>
</Tabs>
...
- Published by the official Foreseerr Helm workflow
- Produced from the selmant/foreseerr repository
- Signed using GitHub’s OIDC identity via Sigstore Fulcio
...
```bash
DIGEST=$(docker buildx imagetools inspect ghcr.io/selmant/foreseerr/foreseerr-chart:0.9.0 --format '{{json .Manifest.Digest}}' | tr -d '"')
...
```bash
DIGEST=$(skopeo inspect docker://ghcr.io/selmant/foreseerr/foreseerr-chart:0.9.0 --format '{{.Digest}}')
...
- [Foreseerr GitHub Repository](https://github.com/selmant/foreseerr)
