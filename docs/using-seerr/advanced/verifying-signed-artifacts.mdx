...
description: Learn how to verify Foreseerr's signed artifacts and SBOM attestations.
---
...
- Docker Hub: `selmantr/foreseerr:<tag>`

You can view all available tags on the [Foreseerr Releases page](https://github.com/selmant/seerr/releases).

---

### Verifying a Specific Release Tag

Each tagged release (for example `v0.1.0-alpha.6`) is immutable and cryptographically signed.
...
```bash
docker buildx imagetools inspect ghcr.io/selmant/seerr:v0.1.0-alpha.6 --format '{{json .Manifest.Digest}}' | tr -d '"'
```
  </TabItem>
...
```bash
skopeo inspect docker://ghcr.io/selmant/seerr:v0.1.0-alpha.6 --format '{{.Digest}}'
```
  </TabItem>
</Tabs>
...
cosign verify ghcr.io/selmant/seerr@sha256:abcd1234... \
  --certificate-identity "https://github.com/selmant/seerr/.github/workflows/release.yml@refs/tags/v0.1.0-alpha.6" \
...
- GitHub Container Registry (GHCR): `ghcr.io/selmant/seerr/foreseerr-chart:<tag>`

You can view all available tags on the [Foreseerr package page](https://github.com/selmant/seerr/pkgs/container/seerr%2Fforeseerr-chart).
...
```bash
docker buildx imagetools inspect ghcr.io/selmant/seerr/foreseerr-chart:3.7.0 --format '{{json .Manifest.Digest}}' | tr -d '"'
```
  </TabItem>
...
```bash
skopeo inspect docker://ghcr.io/selmant/seerr/foreseerr-chart:3.7.0 --format '{{.Digest}}'
```
  </TabItem>
</Tabs>
...
```bash
cosign verify ghcr.io/selmant/seerr/foreseerr-chart@sha256:abcd1234... \
  --certificate-identity "https://github.com/selmant/seerr/.github/workflows/helm.yml@refs/heads/develop" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com"
```

:::info Successful Verification Example
Verification for `ghcr.io/selmant/seerr/foreseerr-chart@sha256:abcd1234...`
...
```bash
DIGEST=$(docker buildx imagetools inspect ghcr.io/selmant/seerr/foreseerr-chart:3.7.0 --format '{{json .Manifest.Digest}}' | tr -d '"')

cosign verify ghcr.io/selmant/seerr/foreseerr-chart@"$DIGEST" \
  --certificate-identity-regexp "https://github.com/selmant/seerr/.github/workflows/helm.yml@refs/heads/develop" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com"

cosign verify-attestation ghcr.io/selmant/seerr/foreseerr-chart@"$DIGEST" \
  --type cyclonedx \
  --certificate-identity-regexp "https://github.com/selmant/seerr/.github/workflows/helm.yml@refs/heads/develop" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com"
```
...
```bash
DIGEST=$(skopeo inspect docker://ghcr.io/selmant/seerr/foreseerr-chart:3.7.0 --format '{{.Digest}}')
...
- [Foreseerr GitHub Repository](https://github.com/selmant/seerr)
