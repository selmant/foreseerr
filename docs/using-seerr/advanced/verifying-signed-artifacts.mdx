...
```bash
docker buildx imagetools inspect ghcr.io/selmant/foreseerr:v0.1.0-alpha.6 --format '{{json .Manifest.Digest}}' | tr -d '"'
```
  </TabItem>
...
```bash
skopeo inspect docker://ghcr.io/selmant/foreseerr:v0.1.0-alpha.6 --format '{{.Digest}}'
```
  </TabItem>
</Tabs>
...
cosign verify ghcr.io/selmant/foreseerr@sha256:abcd1234... \
  --certificate-identity "https://github.com/selmant/foreseerr/.github/workflows/release.yml@refs/tags/v0.1.0-alpha.6" \
...
You can view all available tags on the [Foreseerr package page](https://github.com/selmant/foreseerr/pkgs/container/seerr%2Fforeseerr-chart).
