...
<dict>
  ...
  <array>
    ...
    <string>/opt/seerr/dist/launcher.js</string>
  </array>
...
```bash
NODE_ENV=production pm2 start dist/launcher.js --name seerr
```
...
2. Start seerr with PM2:
```powershell
$env:NODE_ENV = "production"; pm2 start dist/launcher.js --name seerr
```
3. Save the process list:
```powershell
pm2 save
