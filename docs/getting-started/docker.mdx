...
description: Install Foreseer using Docker
sidebar_position: 1
---
# Docker
:::info
...
:::info
An alternative Docker image is available on Docker Hub for this project. You can find it at [Docker Hub Repository Link](https://hub.docker.com/r/selmantr/foreseer)
...
:::info
All Foreseer images are cryptographically signed and include a verified [Software Bill of Materials (SBOM)](https://cyclonedx.org/).
...
## Unix (Linux, macOS)
:::warning
Be sure to replace `/path/to/appdata/config` in the below examples with a valid host directory path. If this volume mount is not configured correctly, your Foreseer settings/data will not be persisted when the container is recreated (e.g., when updating the image or rebooting your machine).
...
```bash
docker stop foreseer && docker rm foreseer
```
...
:::tip
You may alternatively use a third-party updating mechanism, such as [Watchtower](https://github.com/containrrr/watchtower) or [Ouroboros](https://github.com/pyouroboros/ouroboros), to keep Foreseer up-to-date automatically.
...
#### Installation:
Define the `foreseer` service in your `compose.yaml` as follows:
```yaml
---
services:
  foreseer:
...
First, create a volume to store the configuration data for Foreseer using using either the Docker CLI:
```bash
...
Then, create and start the Foreseer container:
<Tabs groupId="docker-methods" queryString>
