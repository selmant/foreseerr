...
description: Install Foreseerr using Docker
sidebar_position: 1
---
# Docker
:::info
...
:::info
An alternative Docker image is available on Docker Hub for this project. You can find it at [Docker Hub Repository Link](https://hub.docker.com/r/selmantr/foreseerr)

Our Docker images are available with the following tags:

- `latest`: Always points to the most recent stable release.
- Version tags (e.g., `v0.10.0`): For specific stable versions.
- Major version aliases (e.g., `v0`): Alias for the latest stable release in the respective major version series.
- Minor version aliases (e.g., `v0.10`): Alias for the latest stable release in the respective minor version series.
- `develop`: Rolling release/nightly builds for using the latest changes (use with caution).
:::

:::info
All Foreseerr images are cryptographically signed and include a verified [Software Bill of Materials (SBOM)](https://cyclonedx.org/).
...
## Unix (Linux, macOS)
:::warning
Be sure to replace `/path/to/appdata/config` in the below examples with a valid host directory path. If this volume mount is not configured correctly, your Foreseerr settings/data will not be persisted when the container is recreated (e.g., when updating the image or rebooting your machine).
...
docker run -d \
  ...
  --cap-drop ALL \
  ghcr.io/selmant/foreseerr:latest
```
...
```bash
docker stop foreseerr && docker rm foreseerr
```
Pull the latest image:
```bash
docker pull ghcr.io/selmant/foreseerr:latest
```
...
:::tip
You may alternatively use a third-party updating mechanism, such as [Watchtower](https://github.com/containrrr/watchtower), to keep Foreseerr up-to-date automatically.
...
#### Installation:
Define the `foreseerr` service in your `compose.yaml` as follows:
```yaml
---
services:
  foreseerr:
    image: ghcr.io/selmant/foreseerr:latest
    init: true
...
First, create a volume to store the configuration data for Foreseerr using using either the Docker CLI:
```bash
docker volume create foreseerr-data
```
...
4. Enter a name for the volume (example: `foreseerr-data`) and hit "Create"

Then, create and start the Foreseerr container:
<Tabs groupId="docker-methods" queryString>
...
To access the files inside the volume created above, navigate to `\\wsl$\docker-desktop-data\version-pack-data\community\docker\volumes\foreseerr-data\_data` using File Explorer.
