...
---
:::info Foreseer
**Foreseer** is a Seerr fork. Config paths stay Seerr-compatible (`CONFIG_DIRECTORY`, Docker `/app/config`). If you already run Seerr, reuse the same config volume. The guide below is upstream Seerr’s Overseerr/Jellyseerr migration; the same steps apply when moving that data onto Foreseer.
:::
