...
const LibraryPosterCard = ({
  ...
  const showOverviewHover = !isBrowse && !isTouch && !isTvShell;
  const showTvPlay = !isBrowse && isTvShell;
      ...
      {({ ref, className }) => (
        ...
        >
          <div className="relative aspect-[2/3] overflow-hidden rounded-xl bg-library-navy ring-1 ring-gray-800">
            ...
            {showTvPlay ? (
              <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex justify-end bg-gradient-to-t from-black/80 to-transparent p-2">
                <span className="inline-flex items-center gap-1 rounded-md bg-indigo-600/90 px-2 py-1 text-xs font-medium text-white">
                  <PlayIcon className="h-4 w-4" />
                  {intl.formatMessage(
                    isResume ? messages.resume : messages.play
                  )}
