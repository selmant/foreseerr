...
const TvDetails = ({ tv }: TvDetailsProps) => {
  ...
  const {
    ...
    tmdbId: data?.id ?? tv?.id,
    mediaType: MediaType.TV,
    title: data?.name ?? tv?.name,
    userId: user?.id,
    onRevalidate: revalidateMedia,
    onBlocklistComplete: closeBlocklistModal,
    initialOnWatchlist: Boolean(tv?.onUserWatchlist),
  });
  ...
  const getAllRequestedSeasons = (is4k: boolean): number[] => {
    ...
    const requestedSeasons = activeRequests.reduce(
    ...
    const episodeRequestedSeasons = new Set(
      ...
      .filter(
        (season) =>
          isSeasonCoveredForFullRequest(
            ...
            episodeRequestedSeasons.has(season.seasonNumber)
        ...
        <div className="media-overview-right">
          <div className="media-facts">
            <RatingBadges
              ...
              tmdbHref={`https://www.themoviedb.org/tv/${data.id}?language=${locale}`}
            />
            {data.originalName &&
