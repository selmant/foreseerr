...
> [!NOTE]
> Foreseerr `v0.8.0` is the current stable release. Upgrades from `v0.1.0`, `v0.2.0`, `v0.2.1`, `v0.3.0`, `v0.4.x`, `v0.5.x`, `v0.6.x`, and `v0.7.x` are supported. Alpha builds (`0.1.0-alpha.x`) are not a supported upgrade source — start from a fresh install or migrate from Seerr (see the migration guide). Back up your configuration before upgrading.
...
docker run -d \
  ...
  ghcr.io/selmant/foreseerr:v0.8.0
```
...
services:
  foreseerr:
    image: ghcr.io/selmant/foreseerr:v0.8.0
...
The image is published at `ghcr.io/selmant/foreseerr`. Prefer an explicit version tag (`v0.8.0`) or a stable alias (`latest`, `v0`, `v0.8`) rather than `develop`.
